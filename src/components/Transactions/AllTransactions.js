...
export default function AllTransactions() {
  ...
  useEffect(() => {
    dispatch(
      ...
      dispatch(updateTotalTransactions(data.payload.totalCount))
    );
  ...
  return (
    <>
      <div style={{ display: "flex" }}>
        ...
        <div style={{ marginRight: "10px" }}>
          <input
            checked={filterMode === "all" ? true : false}
            onChange={handleFilterMode}
            type="radio"
            name="filterType"
            value="all"
      ...
      <div>
        <form onSubmit={handleFormSubmit}>
          <input type="text" size="26" placeholder="Search from tracker..." value={searchText} onChange={handleSearchText} />
        </form>
      </div>
      ...
      <div>{generatePaginationPage()}</div>
    </>
  );
}
